...
class JSONValidationError : public QCCCError
{
public:
	JSONValidationError(QString cause) : QCCCError(cause) {}
};
class FileOpenError : public QCCCError
{
public:
	FileOpenError(const QFile &file) : QCCCError(QObject::tr("Error opening %1: %2").arg(file.fileName(), file.errorString())) {}
};
